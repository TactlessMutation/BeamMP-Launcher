...
void InitOptions(int argc, const char* argv[], Options& options) {
    ...
    while (i < argc) {
        ...
        if (argument == "-p" || argument == "--port") {
            ...
            if (i + 1 >= argc) {
                error("You must specify a path after the `--game-dir` argument");
            } else {
                options.game_dir = argv[i + 1];
                i++;
            }
            ...
            std::cout
                << "USAGE:\n"
                << "\t"
                + std::filesystem::path(options.executable_name).filename().string()
                + " [OPTIONS] [-- <GAME ARGS>...]\n"
                << "\n"
                << "OPTIONS:\n"
                ...
                << "\t--game-dir <path>      Path to BeamNG.drive's game installation directory\n"
